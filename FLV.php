...
class FLV
{
    ...
    public $FLV_HEADER_SIGNATURE = FLV_HEADER_SIGNATURE;

    /** The FLV main header size */
    public $FLV_HEADER_SIZE = FLV_HEADER_SIZE;

    /** The FLV tag header size */
    public $TAG_HEADER_SIZE = FLV_TAG_HEADER_SIZE;

    /**
        Maximun number of bytes to process as tag body. This is a safety meassure against
        corrupted FLV files.
    */
    public $MAX_TAG_BODY_SIZE = FLV_TAG_MAX_BODY_SIZE;
     ...
     * @return bool|true
     */
    public function __construct($fname = false, $timeout = 30)
    {
        if ($fname) {
     ...
     */
    public function open($fname = false, $timeout = 30)
    {
        $this->error = array();
        if ($fname) {
            $this->filename = $fname;
            ...
            if (!empty($url['scheme'])) {
                if (!is_readable(FLV_INCLUDE_PATH) || !is_writable(FLV_INCLUDE_PATH)) {
                    ...
                    if (!$this->_fh) {
                        $this->_die('Unable to Open temporary file');
                    }
                    ...
                    $this->_die('Unable to Make temporary file');
                }
            } else {
                $this->_fh = @fopen($fname, 'r');
                if (!$this->_fh) {
                    $this->_die('Unable to open the file');
                }
            }
            ...
            if (substr($hdr, 0, 3) !== $this->FLV_HEADER_SIGNATURE) {
                $this->_die('The header signature does not match');
            }
    ...
    */
     ...
     */
    public function createMetaData($newMetaData = false, $merge = true)
    {
     ...
     */
    public function playFlv($limitSpeed = 0, $seekat = 0, $newMetaData = false, $merge = true)
    {
        session_write_close();
        ...
        if ($limitSpeed) {
            if ($this->FileInfo['bitrate']) {
                $limitSpeed = ceil(($this->FileInfo['bitrate']/100)/8)+$limitSpeed-1;
            } else {
                $limitSpeed = false;
            }
        }
        if (isset($metadata)) {
            $size = strlen($metadata) + (filesize($this->filename) - ftell($this->_fh)) + 9;
        } else {
            $size = filesize($this->filename) - ftell($this->_fh) + 9;
        }
     ...
     */
    public function playFlvLock($limitSpeed = 0, $seekat = 0, $newMetaData = false, $merge = true, $usedb = false)
    {
        $this->_nocashe = true;
    ...
    */
    public function getFlvThumb($offset=2000, $usemetadata=true)
    {
        session_write_close();
    ...
    */
    public function getFlvPreview($offset=2000, $duration=2000, $usemetadata=true, $speedModifyer=100)
    {
        session_write_close();
        ...
        while ($tag = $this->getTag($skipTagTypes)) {
            if ($tag->type == FLV_TAG_TYPE_VIDEO) {
                ...
                if (!$dataArray && $tag->timestamp >= ($offset+$duration)) {
                    break;
                }
            }
            //Does it actually help with memory allocation?
     ...
     * @param bool       $merge       Merge original array with new one
     */
    public function downloadFlv($limitSpeed = 0, $newMetaData = false, $merge = true)
    {
        session_write_close();
    ...
    */
    public function setHeader($size=0)
    {
     ...
     */
    public function openLock($key=false, $validtime=30, $usedb=false)
    {
        if (!$key) {
            return false;
        }
        ...
        if ($usedb) {
            ...
            while ($row = mysql_fetch_row($result)) {
                if ($row[0] == $tableKey) {
                    $maketableKey = false;
                } elseif ($row[0] == $tableBlock) {
                    $maketableBlock = false;
                }
            }
            ...
            if ($rowKey['uid']) {
                $query = "UPDATE ".$tableKey." SET timeout = $expire , ip = '$ip' WHERE uid = ".$rowKey['uid'];
            } else {
                $query = "INSERT INTO ".$tableKey." VALUES ('','$sid','$key','$expire','$ip')";
            }
            @mysql_query($query);
    ...
    */
    public function closeLock($lock_id = 0)
    {
        if ($lock_id) {
            $this->loadConfig();
            $this->dbConnect();
    ...
    * @return true/false			if sucess/failed.
    */
    public function validLock($usedb=false)
    {
        if (!session_id()) {
            session_start();
        }
        ...
        if ($usedb) {
            ...
            if ($rowKey['uid'] && ($this->config['block_counter'] > $rowBlock['count'] || !$rowBlock['count'])) {
                $return = true;
                ...
                if ($rowBlock['uid']) {
                    $query = "UPDATE ".$tableBlock." SET count = count+1 WHERE uid = ".$rowBlock['uid'];
                } else {
                    $query = "INSERT INTO ".$tableBlock." VALUES ('','$ip','1')";
                }
                @mysql_query($query);
    ...
    */
     ...
     */
    public function getTag($skipTagTypes = false)
    {
        if ($this->eof) {
            return null;
        }
    ...
    */
     ...
     */
    public function defaultMetaData($supressGetId3=false)
    {
        $buffMetaData = array();
    ...
    */
    public function replaceTimestamp(&$frame, $newTimestamp)
    {
     ...
     */
    public function modifyTimestamp($timestamp, $modifyer=100)
    {
        if ($modifyer < 1) {
            $modifyer = 1;
        }
